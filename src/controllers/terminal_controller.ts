...
import {
  DEFAULT_THEME, THEMES, type TerminalTheme
} from "../themes"
...
export default class TerminalController
  ...
  // Stimulus target declarations
  declare authMethodTarget: HTMLSelectElement
  declare connectBtnTarget: HTMLButtonElement
  declare connectionFormTarget: HTMLElement
  declare containerTarget: HTMLElement
  declare contentTarget: HTMLElement
  declare disconnectBtnTarget: HTMLButtonElement
  declare hostTarget: HTMLInputElement
  declare pageTarget: HTMLElement
  declare passwordTarget: HTMLInputElement
  declare passwordGroupTarget: HTMLElement
  declare portTarget: HTMLInputElement
  declare statusTarget: HTMLElement
  declare subtitleActionsTarget: HTMLElement
  declare userTarget: HTMLInputElement
  ...
  declare autoConnectValue: boolean
  declare channelNameValue: string
  declare hostIdValue: string
  declare modeValue: string
  declare savedThemeValue: string
  declare sshAuthMethodValue: string
  declare sshHostValue: string
  declare sshPortValue: string
  declare sshUserValue: string

  private channel: any = null
  private term: Terminal | null = null
  private fitAddon: FitAddon | null = null
  private consumer: any = null
  private ghosttyReady = false
  ...
  changeAuthMethod () {
    if (!this.hasPasswordGroupTarget) return
    if (
      this.authMethodTarget.value === "password"
    ) {
  ...
  async connectTerminal () {
    ...
    currentTerm.onData((data: string) => {
      this.channel?.send({
        type: "input", data
      })
    })

    currentTerm.onResize((
      { cols, rows }:
  ...
  private getSelectedThemeName (): string {
  ...
  private readSshParams () {
    ...
    return {
      ...
      port: parseInt(
        this.sshPortValue || "22", 10
      ),
      user: this.sshUserValue || "root",
      auth_method:
        this.sshAuthMethodValue || "key"
    }
  }
